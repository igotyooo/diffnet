...
function task:getQueries(  )
	return { 2687,2688,2689,2690,2691,2696,2697,2698,2699,2700,2701,2702,2703,2704,2705,2706,2713,2714,2715,1792,1793,1794,1795,1796,1797,1798,1799,1800,1801,1802,1810,1811,1812,1813,1814,1817,1818,1819,3119,3120,3124,3125,3126,3127,3128,3129,3134,3135,3136,3137,3138,3139,3146,3147,3148,3150,547,548,549,553,554,555,558,560,561,564,566,567,1232,1233,1234,1235,1236,1237,1238,1240,1241,1242,1243,1256,1257,1264,613,614,616,617,618,626,627,628,629,631,632,633,634,639,648,651,653,1482,1485,1486,1487,1488,1489,1490,1499,1503,1504,1508,473,474,479,480,481,483,484,485,499,502,504,505,1189,1191,1192,1193,1194,1196,1197,1201,1202,1204,1205,1219,77,78,79,80,83,94,95,104,105,114,115,2687,2688,2689,2690,2691,2696,2697,2698,2699,2700,2701,2702,2703,2704,2705,2706,2713,2714,2715,2716,1792,1793,1794,1795,1796,1797,1798,1799,1800,1801,1802,1810,1811,1812,1813,1814,1817,1818,1819,3120,3124,3125,3126,3127,3128,3129,3134,3135,3136,3137,3138,3139,3146,3147,3148,3150,1758,1759,1760,1761,1762,1763,1764,1765,1766,1770,1771,1773,1775,473,479,480,481,483,484,485,498,499,500,501,502,504,505,1232,1234,1235,1236,1237,1238,1239,1240,1241,1242,1243,1257,1264,666,667,678,679,680,681,682,683,684,685,692,693,1482,1485,1486,1487,1488,1489,1490,1499,1503,1504,613,614,616,617,618,626,628,629,631,632,633,634,651,2950,2951,2952,2953,2954,2955,2958,2961,2973,2974,2976,2979 }
end
...
function task:getFunctionTest(  )
	return
		...
		function( x, y ) return self:aggregateAnswers( x, y ) end,
...
function task:getQuery( queryNumber )
	...
	local vpath = ffi.string( torch.data( self.dbval.vid2path[ vid ] ) )
	local numFrame = self.dbval.vid2numim[ vid ]
	local numChunk = math.min( numFrame, 100000 )
...
function task:aggregateAnswers( answers, queryNumber )
    local dstDir = paths.concat( self.opt.dirRoot, 'pftest/vggm_s' )
    paths.mkdir( dstDir )
    local dstName = string.format( '%06d.txt', queryNumber )
    local dstPath = paths.concat( dstDir, dstName )
    local fp = io.open( dstPath, 'w' )
    local cname = ffi.string( torch.data( self.dbval.cid2name[ self.dbval.vid2cid[ queryNumber ] ] ) )
    local vname = ffi.string( torch.data( self.dbval.vid2path[ queryNumber ] ) )
    vname = vname:match( gpath.db.ucf101_rgb_s1 .. 'jpegs_256/(.+)' )
    fp:write( string.format( 'vname: %s\n', vname ) )
    fp:write( string.format( 'gtcls: %s\n', cname ) )
	for k, v in pairs( answers ) do
        local _, pcids = v:float(  ):sort( 2, true )
		for i = 1, pcids:size( 1 ) do
            local pcid = pcids[ i ][ 1 ]
            local pcname = ffi.string( torch.data( self.dbval.cid2name[ pcid ] ) )
